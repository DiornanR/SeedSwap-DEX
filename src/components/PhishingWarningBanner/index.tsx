...
const Container = styled(Flex)`
  ...
  background: linear-gradient(0deg, rgba(39, 38, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #24a4ab 0%, #004d1a 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #24a4ab 0%, #004d1a 100%);
  }
`
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://seedswap.finance - check the URL carefully.")
    return warningText.split(/(https:\/\/seedswap.finance)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://seedswap.finance'}
          color={text === 'https://seedswap.finance' ? '#FFFFFF' : '#FFFFFF'}
        >
          {text}
        </Text>
      ))}
