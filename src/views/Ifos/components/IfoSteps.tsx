...
const Step1 = ({ hasProfile }: { hasProfile: boolean }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      <Text>
        {t(
          'The number of iSEED equals the locked staking amount if the staking duration is longer than %weeks% weeks. If the staking duration is less than %weeks% weeks, it will linearly decrease based on the staking duration.',
          {
            weeks: weeksDisplay,
          },
        )}
      </Text>
      <InlineLink external href="https://docs.seedswap.finance/products/ifo-initial-farm-offering/iseed">
        {t('Learn more about iSEED')}
      </InlineLink>
    </Box>,
    {},
  )
  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Lock SEED in the SEED pool')}
      </Heading>
      <Box>
        <Text mb="4px" color="textSubtle" small>
          {t(
            'The maximum amount of SEED you can commit to the Public Sale equals the number of your iSEED. Lock more SEED for longer durations to increase the maximum SEED you can commit to the sale.',
          )}
        </Text>
        <TooltipText as="span" fontWeight={700} ref={targetRef} color="textSubtle" small>
          {t('How does the number of iSEED calculated?')}
        </TooltipText>
        <Text mt="4px" color="textSubtle" small>
          {t(
            'Missed this IFO? You will enjoy the same amount of iSEED for future IFOs if your locked-staking position is not unlocked.',
          )}
        </Text>
      </Box>
      {hasProfile && (
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t(
          'Please note that SEED in the fixed-term staking positions will remain locked and can not be used for committing to IFO sales. You will need a separate amount of SEED in your wallet balance to commit to the IFO sales.',
        )}{' '}
        <br />
      </Text>
        ...
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small mb="16px">
              {t('You’ll need an active SeedSwap DEX Profile to take part in an IFO!')}
            </Text>
            {renderAccountStatus()}
          </CardBody>
        )
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent SEED tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
