...
const PoolStatsInfo: React.FC<React.PropsWithChildren<ExpandedFooterProps>> = ({
  ...
  return (
    <>
      {profileRequirement && (profileRequirement.required || profileRequirement.thresholdPoints.gt(0)) && (
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text small textAlign="right">
            {profileRequirement.required && t('Seed Profile')}{' '}
      ...
      {vaultKey && (
        <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <LinkExternal href="https://docs.seedswap.finance/products/pool/new-seed-pool" bold={false} small>
            {t('View Tutorial')}
          </LinkExternal>
        </Flex>
      )}
      ...
      {account && tokenAddress && (
        <Flex justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <AddToWalletButton
            ...
            tokenLogo={`https://tokens.seedswap.finance/images/${tokenAddress}.png`}
          />
        </Flex>
      )}
    </>
