...
export const useBCakeTooltipContent = () => {
  ...
  const tooltipContent = (
    <>
      <Box mb="20px">
        {t(
          'Yield Boosters allow you to boost your farming yields by locking SEED in the fixed-term staking SEED pool. The more SEED you lock, and the longer you lock them, the higher the boost you will receive.',
        )}
      </Box>
      <Box>
        {t('To learn more, check out the')}
        <Link target="_blank" href="https://medium.com/seedswap/introducing-bseed-farm-yield-boosters-b27b7a6f0f84">
          {t('Medium Article')}
        </Link>
      </Box>
    </>
...
const imageSrc = 'boosterCardImage'
...
export const BCakeBoosterCard = () => {
  ...
  return (
    <CardWrapper>
      <ImageWrapper>
        <img src={`${imagePath}${imageSrc}.png`} width={99} height={191} alt={t('')} />
      </ImageWrapper>
      {/*
      <ImageWrapper>
    ...
    return (
      <Box>
        ...
        <Text color="textSubtle" fontSize={12} mb="16px">
          {t('An active fixed-term SEED staking position is required for activating farm yield boosters.')}
        </Text>
        <ConnectWalletButton width="100%" style={{ backgroundColor: theme.colors.textSubtle3 }} />
      </Box>
    )
  if (!locked)
    return (
